...
## 複数ボット編集 — 実装済み 2026-09-03 (#18, #22, #24)

- 全 admin ルーターが `X-Bot-Id` ヘッダで対象ボットを解決（`bot_members` で所属検証）。
  未指定なら従来通り所属ボットを1つ選ぶ（後方互換）
- `GET /api/bot/list` — 自分が所属する全ボット + ロール
- `create_bot` の「既にボットあり」ブロックを撤去。`bots.user_id` の unique 制約も削除（migration 0012）
- フロント: `api.ts` が localStorage の `currentBotId` を `X-Bot-Id` で送信 /
  `AuthContext` に `bots[]` `currentBotId` `setCurrentBot` /
  サイドバーにボット切替セレクタ（「＋ 新しいボット」付き） /
  `BotPage` は `?new=1` で追加作成モード

### 残タスク
- 同時編集の競合は last-write-wins（必要なら `updated_at` で楽観ロック）
- 登録は誰でも可能（招待経由のみに絞るかは別途）
